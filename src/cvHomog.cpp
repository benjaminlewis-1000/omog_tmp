...
******************************************************************/
	...
	cvHomogCalc()	{
		...
		matchedSub = n.subscribe(matchedPointsTopic, 5, &cvHomogCalc::calculate, this);
		...
		directionSub = n.subscribe(directionChangeTopic, 1, &cvHomogCalc::switchNavDirection, this);
		...
		if (ros::param::has("homogControlInputTopic") ){
			...
			homogControlTopic = "cvHomogControlLawInputs";
		}
...
******************************************************************/
	...
	*****************************/
			...
			for(int i = 0 ; i < 3; i++){
				for (int j = 0; j < 3; j++){
					homography.data.push_back(H.at<double>(i, j) );
				}
			}
...
{
  //Initiate ROS
  ros::init(argc, argv, "homographyCV");

  //Create an object of class SubscribeAndPublish that will take care of everything
  cvHomogCalc cvHomogCalcObject;
